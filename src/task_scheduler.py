...
class TaskScheduler:
    ...
    def display_scheduled_tasks(self):
        ...
        sorted_dates = sorted(self.scheduled_tasks.keys())
        first_date = datetime.strptime(sorted_dates[0], "%Y-%m-%d")
        last_date = datetime.strptime(sorted_dates[-1], "%Y-%m-%d")

        rows = [weekdays]

        # Zmienne do przechowywania danych wiersza i indeksu dnia tygodnia
        week_row = [''] * 8  # Zostawiamy pierwsze miejsce na numer wiersza
        row_number = 1  # Dodajemy numerację wierszy
        ...
        while current_date <= last_date:
            ...
            if date_str in self.scheduled_tasks:
                tasks = self.scheduled_tasks[date_str]
                task_names = '\n'.join([task['name'] for task in tasks])
                content = f"{date_str}\n{task_names}"
                week_row[weekday_index + 1] = content  # Dodajemy 1, aby pomieścić numer wiersza
                week_row[0] = row_number  # Ustawiamy numer wiersza

            current_date += timedelta(days=1)

            if current_date.weekday() == 0 or current_date > last_date:
                # Dodajemy wiersz do tabeli tylko jeśli zawiera zadania
                if week_row[0]:
                    rows.append(week_row)
                    row_number += 1

                # Dodajemy pusty wiersz i wiersz z nazwami dni tygodnia po każdym tygodniu
                rows.append([''] * 8)
                rows.append(weekdays)

                week_row = [''] * 8  # Resetujemy wiersz na kolejny tydzień

        # Usuwamy ostatni, niepotrzebny wiersz z nazwami dni tygodnia
        rows.pop()
