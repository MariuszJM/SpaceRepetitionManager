...
class DataPresenter:
    ...
    def prepare_tasks_display_data(
        self, tasks_data: Dict[str, List[Dict]]
    ) -> List[List[str]]:
        ...
        while current_date <= last_date:
            ...
            if date_str in tasks_data:
                ...
                week_row[weekday_index + 1] = self.replace_spaces_with_underscores(
                    content
                )
    ...
    def display_task_delay(
        ...
        print(
            "=" * 13 * 4,
            f"\nTask '{formatted_task_name}' has been delayed by {days_delay} days,",
            f"\nwhich exceeds the allowed range of {allowed_range[0]} to {allowed_range[1]} days.",
            f"\nPropoused task date: {formatted_task_date}.",
        )
    ...
    def format_content(self, date_str: str, tasks_data: List[str]) -> str:
        ...
        if action:
            task_names = ", ".join(
                [item for task in tasks_data for item in task["tasks"]]
            )
    ...
    def get_action(self, tasks_data):
        """Determines the action to be displayed based on task data."""
        if "updated_existing_event" in tasks_data[0]:
            return (
                "UPDATE EVENT"
                if any(item["updated_existing_event"] for item in tasks_data)
                else "DELETE EVENT"
            )
        return ""

    def print_tasks_table(self, tasks_display_data):
        """Prints the tasks table in a formatted grid."""
